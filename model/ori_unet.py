...
from model.ori_unet_parts import *
...
class OriUNet(nn.Module):
    def __init__(self, n_channels, n_classes, param, bilinear=False):
        ...
        if self.net_work_type == 1:
            ...
            self.down4 = (Down(m4, m5 // factor))
            # 上采样模块
            self.up1 = (Up_C(m5, m4 // factor, 12, self.bilinear))
            self.up2 = (Up_C(m4, m3 // factor, 8, self.bilinear))
            self.up3 = (Up(m3, m2 // factor, self.bilinear))
            self.up4 = (Up(m2, m1, self.bilinear))
            self.outc = (OutConv(m1, n_classes))
    ...
    def construction_U_shape(self, input_channels, middle_channels, output_channels):
         # 输出模块
