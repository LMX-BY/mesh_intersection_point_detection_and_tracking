...
from model.DW_LRLS_short_cut_conv import *


class UNetWithDWLRLSConvShortCut(nn.Module):
    def __init__(self, n_channels, n_classes, param, bilinear=False):
        super(UNetWithDWLRLSConvShortCut, self).__init__()
        # 初始化UNet类的属性
        ...
        if self.net_work_type == 0:
            ...
            m4 = m3 * 2
            m5 = m4 * 2
            self.inc = (DoubleDWLRLSConvWithShortCut(n_channels, m1, self.k_size_LSLR))  # 输入模块
            # 下采样模块
            ...
            # 上采样模块
            self.up1 = (Up(m5, m4 // factor, self.k_size_LSLR, self.bilinear))
            self.up2 = (Up(m4, m3 // factor, self.k_size_LSLR, self.bilinear))
            self.up3 = (Up(m3, m2 // factor, self.k_size_LSLR, self.bilinear))
            self.up4 = (Up(m2, m1, self.k_size_LSLR, self.bilinear))
