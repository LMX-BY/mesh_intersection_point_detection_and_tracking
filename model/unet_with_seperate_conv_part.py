...
#             af.act_fn,
#             LRLSConv2dSame(mid_channels, out_channels, k_size, False),
...
#             af.act_fn,
#             LRLSConv2dSame(mid_channels, out_channels, k_size, True),
#             af.act_fn
#         )
#
#     # x，即输入特征图
...
class UpRate(nn.Module):
    """Upscaling then double conv"""

    # 上采样，然后双卷积
    def __init__(self, in_channels, out_channels, UpRate, bilinear=True):
        super().__init__()
        ...
        # 转置卷积
            ...
            self.up = nn.ConvTranspose2d(in_channels, in_channels // UpRate, kernel_size=2, stride=2)
            self.conv = DoubleConv(in_channels, out_channels)
...
class UpLRLSC(nn.Module):
    """Upscaling then double conv"""

    # 上采样，然后双卷积
    def __init__(self, in_channels, out_channels, k_size, C, bilinear=True):
        super().__init__()

        self.up = nn.ConvTranspose2d(in_channels, C, kernel_size=2, stride=2)
        self.conv = DoubleLRLSConv(out_channels*2, out_channels, k_size)
